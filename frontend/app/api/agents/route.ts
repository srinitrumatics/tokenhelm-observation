import { NextResponse } from "next/server";
import { getEventSource } from "@/lib/observation/source";
import { filterByRange } from "@/lib/analytics/overview";
import {
  computeAgentLeaderboard,
  computeAgentDetail,
  computeAgentFlags,
} from "@/lib/analytics/agents";

/**
 * GET /api/agents — Agent analytics / AgentOps (US3).
 * Contract: specs/002-ai-observability-platform/contracts/rest-api.md
 *
 *   ?from=&to=     optional date-range filter (FR-012)
 *   (no agent)     → leaderboard + execution tree + recommendation flags
 *   ?agent=<name>  → detail (timeline, executions, trend, parent/children, attribution)
 *
 * Storage-agnostic: computes over ObservationEvents only; the hierarchy comes from
 * parent_agent/tool_name edges in the events, never from UI structures.
 */

export const dynamic = "force-dynamic";
export const revalidate = 0;

export async function GET(request: Request) {
  try {
    const url = new URL(request.url);
    const from = url.searchParams.get("from");
    const to = url.searchParams.get("to");
    const agent = url.searchParams.get("agent");

    const source = getEventSource();
    const { events, skipped, duplicates, present, source: src } = await source.read();
    const scoped = filterByRange(events, from, to);
    const meta = { source: src, present, skipped, duplicates, generatedAt: new Date().toISOString() };

    if (agent) {
      return NextResponse.json(
        { detail: computeAgentDetail(scoped, agent), meta },
        { headers: { "Cache-Control": "no-store" } },
      );
    }

    const leaderboard = computeAgentLeaderboard(scoped);
    return NextResponse.json(
      { leaderboard, flags: computeAgentFlags(leaderboard), meta },
      { headers: { "Cache-Control": "no-store" } },
    );
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to read event source";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
