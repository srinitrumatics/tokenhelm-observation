...
import { computeAgentLeaderboard, computeAgentFlags } from "@/lib/analytics/agents";
import { AgentLeaderboard } from "@/components/agents/AgentLeaderboard";
import { AgentTree } from "@/components/agents/AgentTree";
import { AgentFlags } from "@/components/agents/AgentFlags";
import { DateRangeFilter } from "@/components/DateRangeFilter";
import { OverviewEmptyState } from "@/components/observability/OverviewEmptyState";

/**
 * Agent analytics / AgentOps (US3). Leaderboard, execution tree (from ObservationEvent
 * relationships), and recommendation flags — recomputed client-side per date range.
 */
export default function AgentsPage() {
  ...
  const leaderboard = useMemo(() => computeAgentLeaderboard(scoped), [scoped]);
  const flags = useMemo(() => computeAgentFlags(leaderboard), [leaderboard]);
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-8">
      <header className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Agent Analytics</h1>
          <p className="text-sm text-slate-400">
            Per-agent cost, failure &amp; hierarchy — derived from{" "}
      ...
      {loading && !data ? (
        <div className="text-sm text-slate-500">Loading agent analytics…</div>
      ) : !hasData ? (
        ...
        <div className="space-y-8">
          <DateRangeFilter
            ...
            onChange={setRange}
          />
          <AgentLeaderboard agents={leaderboard.agents} unattributed={leaderboard.unattributed} />
          <AgentTree tree={leaderboard.tree} />
          <AgentFlags flags={flags} />
        </div>
      )}
    </main>
  );
